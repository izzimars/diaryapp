...
describe('Diary API Tests', () => {
  ...
  describe('POST /api/v1/diary - Create Diary', () => {
    ...
    it('should fail to create diary without authentication', async () => {
      const diaryData = TestHelper.generateDiaryData('Unauthorized Diary', 'This should fail');
  ...
  describe('GET /api/v1/diary - Get User Diaries', () => {
    before(async () => {
      // Create additional diary entries for testing
      const additionalEntries = [
        TestHelper.generateDiaryData('Second Diary Entry', 'Another test entry'),
        TestHelper.generateDiaryData('Third Diary Entry', 'Yet another test entry')
      ];

      for (const entry of additionalEntries) {
        await TestHelper.createDiary(testUser.token!, entry);
      }
    });
    ...
    it('should get all diaries for authenticated user', async () => {
      ...
      response.body.data.forEach((diary: any) => {
        expect(diary.user_id).to.equal(testUser.userId);
      });
    });
  ...
  describe('DELETE /api/v1/diary/:id - Delete Diary', () => {
    ...
    before(async () => {
      ...
      const diaryData = TestHelper.generateDiaryData('Diary to Delete', 'This diary will be deleted in tests');
      const createdDiary = await TestHelper.createDiary(testUser.token!, diaryData);
      diaryToDeleteId = createdDiary.diary_id;
    });
    ...
    it('should delete diary as admin', async () => {
      ...
      const diaryData = TestHelper.generateDiaryData('Admin Delete Test', 'Admin will delete this');
      const createdDiary = await TestHelper.createDiary(testUser.token!, diaryData);

      const response = await request
        .delete(`/api/v1/diary/${createdDiary.diary_id}`)
        .set('Authorization', `Bearer ${testAdmin.token}`)
        .expect(StatusCodes.OK);
